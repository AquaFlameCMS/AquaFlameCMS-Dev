...
elseif ($type == 'forum') {   //Updates and change innerHtml for change order in forums
    ...
    while ($row = mysql_fetch_assoc($sql_forum)) {
        ...
        echo $number_t['count'] . '</td>
          ...
          <td class="inc">';
        if ($i > 1)
            echo'<a href="javascript:;" onclick=move("' . $row['id'] . '","up","forum");><div class="arrow-up"></div></a>';
        if ($i < mysql_num_rows($sql_forum))
            echo '<a href="javascript:;" onclick=move("' . $row['id'] . '","down","forum");><div class="arrow-down"></div></a>';
        '</td>
        </tr>';
    }
    echo '</tbody></table>';
