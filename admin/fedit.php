...
if (isset($_POST['s_categ'])) {
    ...
    if ($change)
        echo '<div align="center"><font color="green">The forum category has been changed</font></div>';
    else
        echo '<div align="center"><font color="red">An error has ocurred, the category could not been changed!</font></div>';
}
elseif (isset($_POST['save'])) {
    ...
    if ($change)
        echo '<div align="center"><font color="green">The changes have been saved!</font></div>';
    else
        echo '<div align="center"><font color="red">An error has ocurred, the changes could not been saved!</font></div>';
}
elseif (isset($_POST['s_delete'])) {
    ...
    if ($option == 'del_all') {
        ...
        if ($del_post && $del_blizz)
            $del_replies = mysql_query("DELETE FROM forum_replies WHERE forumid = '" . intval($_POST['f_id']) . "'"); //delete replies
        ...
        if ($del_post && $del_blizz)
            $del_threads = mysql_query("DELETE FROM forum_threads WHERE forumid = '" . intval($_POST['f_id']) . "'"); //delete topics

        if ($del_threads || mysql_num_rows($threads) < 1)
            $delete = mysql_query("DELETE FROM forum_forums WHERE id = '" . intval($_POST['f_id']) . "'");

        if ($delete)
            die('<div align="center"><font color="green">The forum has been deleted. All the threads have been deleted too!</font></div><meta http-equiv="refresh" content="3;url=fcategory.php"/>');
        else
            echo '<div align="center"><font color="red">An error has ocurred, the forum could not been deleted!</font></div>';
    }
    else {
        mysql_select_db($server_db);
        $delete = mysql_query("DELETE FROM forum_forums WHERE id = '" . intval($_POST['f_id']) . "'");
        if ($delete)
            $change = mysql_query("UPDATE forum_threads SET forumid = '" . $option . "' WHERE forumid = '" . intval($_POST['f_id']) . "'");
        if ($change)
            die('<div align="center"><font color="green">The forum has been deleted. All the threads have been moved!</font></div><meta http-equiv="refresh" content="3;url=fcategory.php"/>');
        else
            echo '<div align="center"><font color="red">An error has ocurred, the forum could not been deleted!</font></div>';
    }
}
                        ...
                        <form method="post" action="" class="styleForm">
                            <input type="hidden" value="<?php echo $forum['id']; ?>" name="f_id">
                                <table width="100%" class="table-content">
                                    <tr>
                                        <td width="50%"><p><strong>Locked: </strong><?php
                                                if ($forum['locked'] == 1)
                                            ...
                                            <p>
                                                ...
                                                <select name="f_categ">
                                                    ...
                                                    while ($op = mysql_fetch_assoc($all_categ)) {
                                                        ...
                                                        if ($op['id'] == $categ['id'])
                                                            echo'<option value="' . $op['id'] . '" selected="selected">' . $op['name'] . '</option>';
                                                        else
                                        ...
                                        <td><p><strong>Delete Forum: </strong>
                                                <select name="f_delete" id="delete">
                                                    ...
                                                    while ($op = mysql_fetch_assoc($all_forum)) {
                                                        if ($op['categ'] != $last_categ)
                                                            echo '<optgroup label="Move to ' . $array_categ[$op['categ']] . '">';
