...
if (isset($_POST['s_new'])) {
    ...
    if (!$categ)
        echo '<div align="center"><font color="red">An error has ocurred, the category could not have been created!</font></div>';
}
elseif (isset($_POST['id_del'])) {
    ...
    if (mysql_num_rows($forums) > 0)
        echo '<div align="center"><font color="red">In order to the delete a category it must be empty. You can delete or move the forums on the forum edit panel.</font></div>';
    else {
        $del = mysql_query("DELETE FROM forum_categ WHERE id = '" . intval($_POST['id_del']) . "'");
        if ($del)
            echo '<div align="center"><font color="green">The forum category have been deleted!</font></div>';
        else
            echo '<div align="center"><font color="red">An error has ocurred, the category could not have been deleted!</font></div>';
    }
}
?>
                    ...
                    <div class="datalist">
                        <div class="heading">
                            ...
                            <form method="post" action="" class="pagination">
                                <input type="text" class="pag" name="new_categ" value="New Category" style="margin: 20px 0 0 29px;width:auto;height:auto;background-color:#FFF;font-size:18px;" onfocus="if (this.value == 'New Category') {
                    this.value = ''
                            ...
                            <table>
                                <thead>
                                    <tr>
                                        ...
                                        <th class="inc"><strong>N� Forums</strong></th>
        ...
        <tr>
          ...
          <td class="desc">';
                                        ...
                                        while ($forum = mysql_fetch_assoc($forums)) {
                                            ...
                                            if ($forum['locked'] == '1')
                                                $f_list = $f_list . '<strong style=color:red;>' . $forum['name'] . '</strong>';
                                            else
                                                $f_list = $f_list . '<strong style=color:green;>' . $forum['name'] . '</strong>';
                                        }
          ...
          <td class="inc">';
                                        if ($i > 1)
                                            echo'<a href="javascript:;" onclick=move("' . $row['id'] . '","up","categ");><div class="arrow-up"></div></a>';
                                        if ($i < mysql_num_rows($sql_categ))
                                            echo '<a href="javascript:;" onclick="move(&#39;' . $row['id'] . '&#39;,&#39;down&#39;,&#39;categ&#39;);"><div class="arrow-down"></div></a>';
                                        '</td>
        </tr>';
                                    }
                                    ?>
