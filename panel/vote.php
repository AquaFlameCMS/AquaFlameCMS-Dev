<div class="sidebar-module " id="sidebar-expansion" style="">
    ...
    <div class="sidebar-content">
        ...
        <ul>
            ...
            while ($vote = mysql_fetch_array($votes)) {
                ...
                if (mysql_num_rows($votedx) > 0) {
                    ...
                    if (time() >= $whenIcanvote)
                        ...
                        if ($timp['ore'] > 0)
                            if ($timp['ore'] > 1)
                                $in_time = '' . $timp['ore'] . ' hours';
                            else
                                $in_time = '' . $timp['ore'] . ' hour';
                        else if ($timp['min'] > 0)
                            if ($timp['min'] > 1)
                                $in_time = 'in ' . $timp['min'] . ' minutes';
                            else
                                $in_time = '' . $timp['min'] . ' minute';
                        else if ($timp['sec'] > 0)
                            if ($timp['sec'] > 1)
                                $in_time = 'in ' . $timp['sec'] . ' seconds';
                            else
                                $in_time = '' . $timp['sec'] . ' second';
                        else
                            $voteable = 1;
                    }
                }
			...
			<li>';
                if ($voteable == 1)
                    echo '<a target="_blank" href="' . $vote['Link'] . '" onclick="window.location = \'vote.php?id=' . $vote['ID'] . '\'" rel="np">';
                echo'
			<span class="info-wrapper clear-after">
			<span class="title">' . $vote['Name'] . '</span></br>';
                if ($voteable == 1)
                    echo '<span class="date">' . $Vote['CanVoteNow'] . '</span>';
                else
                    echo '<span class="date">' . $Vote['CanVoteIn'] . '' . $in_time . '</span></br>
