var GalleryViewer = {
    ...
    imageLoader: function(loadingImage) {
        ...
        if (loadingImage.complete) {
            ...
            GalleryViewer.currentImage.style.display = "none";
            GalleryViewer.imageTimeout = setTimeout(function() {
                GalleryViewer.imageLoader(loadingImage)
            }, 300);
        }
    },
    ...
    setImageBackground: function(backgroundImage, target) {
        ...
        if (backgroundImage.complete) {
            ...
            setTimeout(function() {
                GalleryViewer.setImageBackground(backgroundImage, target)
            }, 100);
        }
...
$(document).keydown(function(currentEvent) {
    if (GalleryViewer) {
        ...
        if (!($(currentTarget).is("textarea") || $(currentTarget).is("input[type='text']"))) {
            if (keyNum == 37) {
                if (viewType == "film-strip")
                    GalleryViewer.getPreviousItem();
                else if (viewType == "thumbnail-page")
                    GalleryViewer.getPreviousPage();
            } else if (keyNum == 39) {
                if (viewType == "film-strip")
                    GalleryViewer.getNextItem();
                else if (viewType == "thumbnail-page")
                    GalleryViewer.getNextPage();
            }
        }
    }
});